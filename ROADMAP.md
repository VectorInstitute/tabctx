...
`CHANGELOG.md` — read those first. Everything below is ranked by priority,
...
(`v0.5.0.json` — concurrency sweep, `v0.5.0-feature-sweep.json` — column-count
sweep) are real, measured numbers, not estimates — treat them as ground
truth for "did a change actually help," and re-run them after any change
that touches concurrency or the estimator.
...
`benchmarks/baselines/v0.5.0.json`) — the obvious next move is "add more Ray
Serve replicas." **That doesn't work today.** Ray Serve's default request
routing has no session affinity (confirmed: `request_router_config` in this
org's existing Helm charts only sets a stats timeout, no affinity strategy).
With 2+ replicas, a `predict()` call for an existing `dataset_id` has no
guarantee of landing on the replica that `fit()` it — callers would see
intermittent, spurious 404s as traffic bounces between replicas that don't
share state. **The current architecture only works correctly as exactly one
replica.** This wasn't caught until deep into scale-testing this session —
don't repeat that: any capacity-planning advice involving replica count is
wrong without this fixed first.
...
**Where to start:**
- Ray Serve supports custom request routing (`request_router_config` /
...
not just a missing nicety — rank it above pure performance work for that
reason.
...
## Priority 3: Cross-request batching — investigate feasibility before committing
...
backend call; N concurrent calls near that ceiling could jointly exceed
...
and reloading) is a bigger, possibly not-worth-it lift; scope this
...
- **TabPFN backend.** `TabularICLBackend` (protocol in `backends/base.py`)
  ...
  cache, or estimator. Real work, but breadth, not depth — do this once
...
- **PyPI publishing.** The GKE deploy still ships a hand-built wheel via
  ConfigMap (see "Where things live" below) — fine for now, but a real
  distribution story matters once this has users outside this org.
...
- `VectorInstitute/inference-platform`, branch `test/tabicl-gke-onboard`,
  ...
  deploys tabctx via a ConfigMap-mounted wheel + `runtime_env.pip` (no
...
- GCP project `agentic-ai-evaluation-bootcamp`, zone `us-central1-f`: where
  ...
  `us-central1-c`, hit a capacity stockout — `us-central1-f` is the
  known-good choice). **Check `gcloud container clusters list` before
  assuming nothing is running** — ephemeral test clusters cost real money
  while alive.
